...
function ProfileScreen() {
  ...
  const [bgImg, setBgImg] = useState('https://wallpapersmug.com/large/fc8c6a/beautiful-girl-anime-art-original-22.jpg');
  React.useEffect(() => {
    const getPost = async () => {
      try{
  ...
  return (
    <Container fluid className={classes.container}>
      ...
      <Container fluid className={classes.subContainer}>
        ...
        <Flex sx={{ justifyItems: 'center', flexDirection: 'column', alignItems: 'center' }}>
          <Flex sx={{ flexDirection: 'row', justifyContent: 'flex-start', gap: '16px', width: '60%' }}>
            <StyledLabel title={'Coin'} color='black' />
            <StyledLabel title={profile?.coint || 0} color='black' />
          </Flex>
          <Flex sx={{ flexDirection: 'row', justifyContent: 'space-between', width: '60%' }}>
            <Flex gap={'6px'}>
        ...
        <Flex direction={'column'} gap={'16px'}>
          ...
          TODO: Chọn danh hiệu
          <Flex direction={'column'} gap={'16px'} sx ={{alignItems: 'center', justifyContent: 'center'}}>
